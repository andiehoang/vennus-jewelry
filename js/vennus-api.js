...
/* ---------- Shared "apply one editable image/video" renderer ----------
   ...
   Hero and category tiles are locked to a fixed shape (16:9 for the
   hero, square for category tiles — see LOCKED_ASPECT in the admin
   bar's crop tool), so their container keeps a real CSS aspect-ratio
   and the image covers it — matching what a locked crop already
   produces, and safely reserving the right amount of space even
   before the image has actually loaded. Editorial photos and menu
   features are free-shaped, so they display at their own natural
   size instead of being cover-clipped into a guessed box. */
const VENNUS_NATURAL_SIZE_KEYS = new Set([
...
window.vennusApplyEditableImage = function (el, media) {
  ...
  const natural = VENNUS_NATURAL_SIZE_KEYS.has(key);
...
/* Applies a settings object to the page — announcement bar, every
   editable image/video slot, theme colours, SEO description. Called
   with a cached copy (if any) for an instant first paint, then again
   with whatever the live backend actually returns. */
function vennusApplySettings(s) {
  if (!s) return;
...
const VENNUS_CACHE_KEY = "vennus_cache_v1";
function vennusReadCache() {
  try { return JSON.parse(localStorage.getItem(VENNUS_CACHE_KEY)) || {}; } catch { return {}; }
}
function vennusWriteCache(patch) {
...
async function vennusLoadCatalogue() {
  ...
  // sleeps after inactivity and can take a real 30-60s to wake back up
  // on the next request, and this site has no build step to bake the
  // latest values into the HTML directly. Without a cache, that cold
  // start is exactly what shows up as "the hero flashes an empty
  // placeholder for ages on reload." A cached copy means repeat
  // visitors (which, in practice, is almost everyone after the first
  // load) see the real content instantly, with the fresh fetch below
  // just quietly confirming or updating it in the background.
  const cache = vennusReadCache();
  if (cache.products && Array.isArray(cache.products) && cache.products.length) {
    window.VENNUS_PRODUCTS = cache.products;
  }
  if (cache.settings) vennusApplySettings(cache.settings);
  ...
  try {
    ...
    vennusApplySettings(s);
    vennusWriteCache({ settings: s });
  } catch { /* settings are optional — a cached copy, if any, is already applied */ }
}
